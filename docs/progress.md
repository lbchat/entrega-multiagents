...
- **Limitação do sentimento histórico (Etapa 6):** o backtest (`run_backtest`, `strategy.py`) usa `sentiment_score=0.5` (neutro) fixo para todas as datas, porque o sentimento de notícias via RSS só existe para o presente — não há como obter notícias históricas de datas passadas pelos feeds atuais. Isso significa que os resultados do backtest avaliam principalmente a qualidade dos indicadores técnicos (RSI, MACD) como sinais de decisão, não a estratégia completa (que em produção também usa sentimento real). Documentado também em célula Markdown no notebook `04_backtest_and_evaluation.ipynb`.
